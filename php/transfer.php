...
$prefillName = isset($_GET['name']) ? $_GET['name']          : '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!is_numeric($amount_raw) || (float)$amount_raw <= 0) {
        ...
        $error = 'Amount too large. Max is ₹99,999.99.';
    } elseif ($receiver_id <= 0) {
        $error = 'Please enter a valid recipient user ID.';
        ...
        try {
            ...
            if (!$sender) throw new RuntimeException('Sender account not found.');
            ...
            $receiver = $r->fetch();
            if (!$receiver) throw new RuntimeException('Recipient not found. Check the user ID.');
            ...
            $pdo->prepare("INSERT INTO transactions (sender_id, receiver_id, amount, comment) VALUES (?,?,?,?)")
                ->execute([$sender_id, $receiver_id, $amount, $comment !== '' ? $comment : null]);
...
// ── GET: load receiver preview & balance ───────────────────
$receiver_preview = null;
if ($prefillId > 0) {
    $s = $pdo->prepare("SELECT id, username FROM users WHERE id = ?");
    $s->execute([$prefillId]);
    $receiver_preview = $s->fetch();
}
$myBalance = $pdo->prepare("SELECT balance FROM users WHERE id = ?");
$myBalance->execute([$_SESSION['user_id']]);
$myBalance = $myBalance->fetchColumn();
    ...
    <form method="POST" action="/transfer.php" id="transfer-form" novalidate>
      ...
      <div class="form-group">
        ...
        <div id="receiver-preview" class="receiver-preview <?= $receiver_preview ? 'visible' : '' ?>">
          <span>👤</span>
          <span id="preview-name"><?= $receiver_preview ? h($receiver_preview['username']) : '' ?></span>
          <span id="preview-id" class="preview-id"><?= $receiver_preview ? '#'.(int)$receiver_preview['id'] : '' ?></span>
        </div>
      ...
      <div class="form-group">
        ...
        <input type="number" id="amount" name="amount" min="0.01" max="99999.99"
               step="0.01" placeholder="0.00"
               value="<?= h($_POST['amount'] ?? '') ?>"
               required class="input-field input-amount">
        <small class="hint">Max: ₹99,999.99</small>
      </div>
  ...
  <div class="transfer-sidebar">
    <div class="card tip-card">
      ...
      <ul>
        ...
        <li>Comment is visible only to the receiver</li>
      </ul>
    </div>
    <a href="/search.php"  class="btn btn-outline btn-full mt-8">🔍 Search for a User</a>
...
document.getElementById('transfer-form').addEventListener('submit', function(e) {
  ...
  if (isNaN(amount) || amount <= 0) { e.preventDefault(); return; }
  if (!confirm(`Send ₹${amount.toFixed(2)} to ${name}?\nThis cannot be reversed.`)) {
    e.preventDefault();
  }
});
</script>
